...
Today the executable code validates immutable in-memory pool snapshots and deterministically selects the best exact-replayed single path found within explicit hop and edge-expansion limits. Separate interruption APIs check caller control only at deterministic pre-expansion boundaries. At an interruption boundary they expose only a fully exact-replayed incumbent or a typed no-plan outcome; callback failures are typed control errors. Opaque process-local checkpoint tokens can resume and branch the same captured search under nondecreasing absolute cumulative work caps without accepting replacement snapshots or requests. RouteLab can also serialize canonical v1 snapshot financial content, create and replay-verify canonical router runs/cases, and feed three fixed offline cases through a deterministic benchmark CLI that separates raw semantic evidence from non-statistical observations. The general router still treats checksums as pinned identity; verification is enforced by the canonical-run boundary. Exact financial, graph, router, interruption/resume, checksum, record, reader, fixture, and harness behavior has focused and independent oracle evidence. Split allocation, unrestricted global optimality, serialized or persisted checkpoints, cross-process resume, wall-clock deadlines, statistical benchmarking, output persistence, comparison modes, services, and protocol adapters are not implemented. The offline deterministic demo reports capability status only; it does not execute a financial request.
