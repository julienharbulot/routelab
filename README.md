...
The current release target is deterministic offline exact-input routing over immutable snapshots. Milestones 0–6 are integrated and cumulatively reviewed complete for their accepted gates. The independent cumulative Milestone 7 review also concluded complete with no severity finding. It mapped exact replay consolidation, the accepted [path-level numerical allocation contract](docs/adr/accepted/0004-path-level-numerical-allocation.md), proposal/reconstruction, the direct numerical runtime, the [retained identical-input evaluation](datasets/evaluations/ethereum-mainnet-uniswap-v2/block-19000000/core12-v1/synthetic-exhaustive-v1/numerical-path-shadow-price-v1/README.md), both acceleration profiles, independent evidence, compatibility, exact commits/CI, and bounded limitations to every Milestone 7a/7b outcome and gate clause. The numerical artifact records primary mode under four mechanically derived clauses while preserving exact replay and the baseline; it creates no default, performance, or global-optimality result. Milestone 7b retains both the negative [first profile](datasets/profiles/ethereum-mainnet-uniswap-v2/block-19000000/core12-v1/synthetic-exhaustive-v1/numerical-path-shadow-price-baseline-v1/README.md) and [supported-regime follow-up](datasets/profiles/ethereum-mainnet-uniswap-v2/block-19000000/core12-v1/supported-regime-suite-v1/numerical-path-shadow-price-preacceleration-v1/README.md). The follow-up corrects the absent attribution population without rewriting the first result, but candidate-set discovery leads none of 12 profiles. Its frozen rule therefore selects no acceleration. Milestone 8 thin-boundary work follows only after the exact Milestone 7 completion-commit gate; learned ordering remains later gated work.
