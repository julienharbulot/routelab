...
Today the executable code validates immutable in-memory pool snapshots and deterministically selects the best exact-replayed single path found within explicit hop and edge-expansion limits. It can also enumerate bounded canonical sets of pairwise pool-disjoint routes as structural proposals; these sets contain no amounts, outputs, receipts, replay authorization, objective, or split plan. Separate interruption APIs expose only exact-replayed incumbents at deterministic boundaries, support opaque process-local resume, and optionally use an injected monotonic bigint deadline clock without putting clock data into checkpoints or semantic hashes. RouteLab also creates and replay-verifies canonical router runs/cases and feeds fixed offline cases through a deterministic benchmark CLI that separates raw semantics from non-statistical observations. Exact financial, graph, router, interruption/resume/deadline, structural-set, record, fixture, and harness behavior has focused and independent oracle evidence. Split allocation/replay, unrestricted global optimality, serialized checkpoints, cross-process resume, default host-clock integration, statistical benchmarking, output persistence, comparison modes, services, and protocol adapters are not implemented. The offline deterministic demo reports capability status only; it does not execute a financial request.
