...
Today the executable code validates immutable in-memory pool snapshots and deterministically selects the best exact-replayed single path found within explicit hop and edge-expansion limits. It can serialize canonical v1 snapshot financial content, compute or verify its prefixed SHA-256 checksum, and create a checksum-verified canonical in-memory bounded-router run with a determinism hash. The general router still treats checksums as pinned identity; verification is enforced by the canonical-run boundary. Exact pool/replay/search/router behavior, focused tests, and independent bounded financial, graph, router, checksum, and run-record oracles are public. Split allocation, unrestricted global optimality, checkpoint/resume and wall-clock deadlines, canonical run parsing or persistence, benchmark tooling, services, and protocol adapters are not implemented. The offline deterministic demo reports capability status only; it does not execute a financial request.
