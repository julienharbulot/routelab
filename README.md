...
Today the executable code validates immutable in-memory pool snapshots and deterministically selects the best exact-replayed single path found within explicit hop and edge-expansion limits. Separate interruption APIs check caller control only at deterministic pre-expansion boundaries. At an interruption boundary they expose only a fully exact-replayed incumbent or a typed no-plan outcome. Opaque process-local checkpoint tokens can resume and branch captured search under nondecreasing absolute cumulative work caps. Optional deadline adapters use only a mandatory injected monotonic bigint clock at those same boundaries; they are cooperative operational controls, not hard real-time preemption. Clock data never enters checkpoint state or semantic hashes, and deterministic APIs remain unchanged. RouteLab can also create and replay-verify canonical router runs/cases and feed three fixed offline cases through a deterministic benchmark CLI that separates raw semantic evidence from non-statistical observations. Exact financial, graph, router, interruption/resume/deadline, record, fixture, and harness behavior has focused and independent oracle evidence. Split allocation, unrestricted global optimality, serialized/persisted checkpoints, cross-process resume, default host-clock integration, statistical benchmarking, output persistence, comparison modes, services, and protocol adapters are not implemented. The offline deterministic demo reports capability status only; it does not execute a financial request.
...
pnpm benchmark    # Replay fixed offline cases and report separated observations.
