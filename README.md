...
Today the executable code validates immutable in-memory pool snapshots and deterministically selects the best exact-replayed single path found within explicit hop and edge-expansion limits. A separate interruptible router checks caller control only at deterministic pre-expansion boundaries. At an interruption boundary it exposes only a fully exact-replayed incumbent or a typed no-plan outcome; callback failures are typed control errors. It can also serialize canonical v1 snapshot financial content, create and replay-verify canonical router runs/cases, and feed three fixed offline cases through a deterministic benchmark CLI that preserves raw semantic evidence separately from single-run timing and environment observations. Those observations are explicitly non-statistical and support no performance conclusion. The general router still treats checksums as pinned identity; verification is enforced by the canonical-run boundary. Exact financial, graph, router, interruption, checksum, record, reader, fixture, and harness behavior has focused and independent oracle evidence. Split allocation, unrestricted global optimality, resumable or serialized checkpoints, wall-clock deadlines, statistical benchmarking, output persistence, comparison modes, services, and protocol adapters are not implemented. The offline deterministic demo reports capability status only; it does not execute a financial request.
