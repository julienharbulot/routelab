...
Milestones 0–5 are integrated and cumulatively reviewed complete for their accepted gates. Milestone 6 is integrated and cumulatively reviewed complete for the additive composed-runtime prerequisite, domain parsing before preparation, accepted historical source contract, canonical snapshot, separately versioned result-blind synthetic corpus, checksummed composed-runtime profile ladder, and fully retained/replayable offline evaluation; its exact completion commit passed CI. Milestone 7a is active. The duplicated legacy and prepared exact replay paths are consolidated behind one internal resolver-parameterized kernel, and that exact commit passed CI. [ADR 0004](docs/adr/accepted/0004-path-level-numerical-allocation.md) freezes the additive proposal-only numerical model, exact reconstruction and authorization boundary, controls, diagnostics, compatibility, independent evidence, and primary-versus-experimental decision rule. The isolated internal proposal/reconstruction core and independent bounded evidence are integrated; it emits no executable allocation and cannot authorize an incumbent. Runtime composition, residual assignment, exact proposal scoring, full-input authorization replay, and diagnostics remain the next release gate. No acquisition client, historical algorithm comparison, performance conclusion, split checkpoint/resume, default router clock, service, protocol adapter, supported numerical allocator, or learned ordering exists.
...
The next Milestone 7a task composes the internal proposal core into the prepared request runtime. It must resolve and validate canonical pool-disjoint candidates, meter proposal and per-iteration work without recharge, assign residual units through exact option replay, reconstruct an exact-sum allocation, and require a distinct full-input exact authorization replay before any incumbent replacement. Every stop or numerical failure preserves the existing exact incumbent. Identical-input Milestone 6 evaluation remains a later same-milestone gate; no approximate value may authorize a result.
...
- Split routing is limited to pool-disjoint routes and configured no-split/equal/chunk-greedy policies. The internal numerical core produces proposal weights and exact bases plus an unassigned residual, but it is not composed into routing and exposes no executable allocation or authorization path. Greedy parts and evaluation work are explicitly bounded; integer flooring and zero-output eligibility can make unit chunks miss the tiny exhaustive optimum. No global allocation or route optimality is claimed, and RouteLab does not submit transactions, hold funds, or model a deployed protocol.
