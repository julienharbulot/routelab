...
Milestones 0–5 are integrated and cumulatively reviewed complete for their accepted gates. Milestone 6 implementation is integrated, and its cumulative review concluded complete for the additive composed-runtime prerequisite, domain parsing before preparation, accepted historical source contract, canonical snapshot, separately versioned result-blind synthetic corpus, checksummed composed-runtime profile ladder, and fully retained/replayable offline evaluation. The review mapped every roadmap outcome and gate clause to current code, immutable artifacts, independent evidence, exact prerequisite commits and CI, and the limitations below. This completion record remains the active release gate until it is integrated and its exact commit passes CI; only then does Milestone 7a become eligible. No acquisition client, historical algorithm comparison, performance conclusion, split checkpoint/resume, default router clock, service, protocol adapter, numerical allocator, or learned ordering exists.
...
After this completion record is integrated and its exact commit passes CI, Milestone 7a path-level numerical allocation is the next eligible work. It must add an approximate shadow-price proposal stage over the bounded pool-disjoint candidate set, reconstruct a deterministic nonnegative `bigint` allocation whose legs sum exactly to the request, resolve residual units with exact evidence, and require a distinct full-input exact authorization replay before incumbent replacement. No-split, equal, greedy, and tiny exhaustive modes remain fallbacks/references. The gate requires independent tiny-exhaustive agreement within documented limits, arbitrary-precision exact sums, fallback preservation on every failure, retained convergence/residual diagnostics, and identical-input comparison against the Milestone 6 equal/greedy baseline without a global-optimality claim.
