...
The next Milestone 7a task implements only the isolated route-curve, deterministic shadow-price proposal, exact IEEE reconstruction, and `bigint` base/residual core frozen by ADR 0004. Independent bounded evidence must derive the expected coefficients, normalized binary64 patterns, solver outcomes, exact sums, and retained exhaustive gaps without importing production helpers. Runtime composition, residual option replay, final authorization, and identical-input Milestone 6 evaluation remain later same-milestone gates; no approximate value may authorize a result.
...
- Split routing is limited to pool-disjoint routes and configured no-split/equal/chunk-greedy policies. ADR 0004 defines a later additive numerical mode, but no numerical allocator is implemented yet. Greedy parts and evaluation work are explicitly bounded; integer flooring and zero-output eligibility can make unit chunks miss the tiny exhaustive optimum. No global allocation or route optimality is claimed, and RouteLab does not submit transactions, hold funds, or model a deployed protocol.
