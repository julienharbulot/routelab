...
The next Milestone 7a task freezes and runs a separately versioned, identical-input evaluation of the exact baseline and numerical runtime over the retained Milestone 6 corpus. It must preserve deterministic semantic results separately from observations, report exact output/work/convergence and qualified tiny-exhaustive gaps, and apply ADR 0004's primary-versus-experimental decision rule without tuning the frozen cohort after results. A negative result is retained honestly and cannot weaken the exact baseline.
...
- Split routing is limited to pool-disjoint routes and configured no-split/equal/chunk-greedy baselines plus an additive direct source-module numerical option. The numerical path is deterministic and exactly authorized, but it has not yet passed identical-input retained evaluation, is not a default mode, and has no canonical run/case schema. Greedy and numerical work are explicitly bounded; integer flooring, zero-output eligibility, and approximate proposal limits can miss the tiny exhaustive optimum. No global allocation or route optimality is claimed, and RouteLab does not submit transactions, hold funds, or model a deployed protocol.
