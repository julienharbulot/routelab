...
The additive path-level numerical mode follows the same stricter separation. Its floor-free continuous curves and binary64 shadow-price weights are proposals only. It decodes the proposed IEEE bit patterns into exact integer weights, reconstructs nonnegative `bigint` base allocations, assigns every residual unit through metered exact partial split replay, and obtains a nonauthorizing exact full-input score. A strictly better score still requires a separate metered fresh full-input replay before incumbent replacement. Any model, convergence, reconstruction, residual, replay, work, interruption, deadline, callback, or clock failure preserves the last fully authorized exact incumbent, which cannot be worse than the exact baseline. [ADR 0004](adr/accepted/0004-path-level-numerical-allocation.md) fixes the complete numerical procedure and failure contract.
...
The additive numerical runtime extends that same request ledger only with `numerical-proposal`, `numerical-iteration`, `numerical-residual-replay`, and `numerical-authorization-replay`. Baseline authorization precedes those stages. The four numerical kinds keep separate safe-integer caps/counters, and the exact-replay kinds keep separate rejection counters; none is exchangeable through a combined work score. Existing composed-runtime and canonical split-v1 counters remain unchanged.

At each discretionary unit, the runtime first determines unit availability or stage completion, then applies that work kind's cumulative cap, checks interruption, samples the absolute monotonic deadline, and finally executes the unit atomically. This boundary applies before every path expansion, best-single replay, set expansion, equal proposal replay, greedy option replay, final baseline authorization replay, numerical proposal, numerical iteration, numerical residual replay, and numerical authorization replay. A cap on one of the six baseline kinds closes only that baseline stage. By the additive numerical contract, a cap on any numerical kind closes the remaining numerical stage; the last fully authorized exact incumbent remains available. Either cap condition contributes `work-limit`; interruption or deadline stops the request. No stop exposes a partial replay or plan.
